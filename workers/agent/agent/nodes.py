...
async def _ddgs_search(query: str, max_results: int):
    """Direct call to DDGS (with retry for network errors)."""
    return await asyncio.to_thread(lambda: list(DDGS().text(query, max_results=max_results, backend="auto")))


async def safe_ddgs_search(query: str, max_results: int):
    """Wrapper: returns an empty list on any error."""
    try:
...
def safe_node(async_func):
    """Decorator to catch exceptions at the graph node level."""
...
async def classify_intent_node(state: AgentState) -> dict[str, Any]:
    ...
    prompt = ChatPromptTemplate.from_messages(
        [
            (
                "system",
                """Определи тип вопроса пользователя. 
    ...
    return {"intent": intent, "current_step_message": f"intent_{intent}"}
...
async def simple_response_node(state: AgentState) -> dict[str, Any]:
    ...
    prompt = ChatPromptTemplate.from_messages([("system", system_msg), ("human", "{question}")])

    chain = prompt | llm | StrOutputParser()
    response = await safe_llm_ainvoke(chain, {"question": state.get("question")})

    return {"final_answer": response, "current_step_message": StepCode.FINALIZING.value}
...
async def internal_search_node(state: AgentState) -> dict[str, Any]:
    ...
    return {"internal_context": new_findings, "current_step_message": msg_code}
...
async def evaluate_node(state: AgentState) -> dict[str, Any]:
    ...
    if not combined.strip() or len(combined) < 50:
        return {"is_sufficient": False, "current_step_message": StepCode.EVALUATING.value}
    ...
    return {"is_sufficient": sufficient, "current_step_message": StepCode.EVALUATING.value}
...
async def reformulate_node(state: AgentState) -> dict[str, Any]:
    ...
    prompt = ChatPromptTemplate.from_messages(
        [
            (
                "system",
                f"Ты — эксперт по поисковым запросам. Сегодня: {current_date_str}. Верни ТОЛЬКО запрос.",
            ),
            ("human", f"{instruction}\n\nВопрос: {{question}}"),
        ]
    )
...
async def web_search_node(state: AgentState) -> dict[str, Any]:
    ...
    translate_prompt = ChatPromptTemplate.from_messages(
        [("system", "Translate to English. Return ONLY the translation."), ("human", "{query}")]
    )
    ...
    return {"web_context": [search_result_text], "current_step_message": StepCode.WEB_FOUND.value}
...
async def synthesize_node(state: AgentState) -> dict[str, Any]:
    ...
    return {"draft_answer": draft, "current_step_message": StepCode.SYNTHESIZING.value}
...
async def finalize_node(state: AgentState) -> dict[str, Any]:
    ...
    return {"final_answer": final, "current_step_message": StepCode.FINALIZING.value}
