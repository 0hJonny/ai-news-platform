...
class ChatRepository:
    ...
    async def ensure_session(self, session_id: str, user_id: str):
        ...
        async with httpx.AsyncClient(timeout=self.timeout) as client:
            response = await client.post(f"{self.base_url}/chats/sessions", json=payload, headers=headers)
    ...
    async def save_message(
        ...
        async with httpx.AsyncClient(timeout=self.timeout) as client:
            response = await client.post(f"{self.base_url}/chats/messages", json=payload, headers=headers)
            response.raise_for_status()
            return response.json()

    async def set_feedback(
        self, user_id: str, message_id: str, rating: str, comment: str | None = None
    ) -> dict:
        """
        ...
        headers = {"X-User-ID": user_id}
        payload = {"message_id": message_id, "rating": rating, "comment": comment}

        async with httpx.AsyncClient(timeout=self.timeout) as client:
            # Pay attention to the URL.
            # Make sure this exact route is registered in Go
            response = await client.post(f"{self.base_url}/chats/feedback", json=payload, headers=headers)
