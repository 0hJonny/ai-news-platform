...
type Config struct {
	...
	NewsHTTPAddr   string `env:"NEWS_HTTP_ADDR" env-default:"http://localhost:5000"`

	AllowedOrigins string `env:"ALLOWED_ORIGINS" env-default:"http://localhost:3000,http://localhost:5173"`
	// Future Redis configuration (comment out or leave on defaults for now)
