-- +goose Up
-- The producer now inserts a draft row (source_link + language only,
-- status PENDING_PARSING) before the parsing task has fetched anything —
-- author/body aren't known yet at that point, so they can no longer be
-- NOT NULL. The parsing task fills them in via PATCH /p/articles/:id once
-- it actually scrapes the page.
